 ...
 */
export function MbahgptCategory({ site, category, pages, categories, reviewCounts, sort, user }: CategoryTemplateProps) {
  return (
    <div data-template="mbahgpt" className="flex min-h-screen flex-col bg-background text-foreground">
      <MbahgptHeader user={user} brand={siteBrand(site)} categories={categories} currentCategorySlug={category.slug} />
        ...
        <section className="mx-auto w-full max-w-3xl px-4 py-6 sm:py-10">
          {pages.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[var(--border)] px-6 py-16 text-center">
              <p className="text-[var(--muted)]">Belum ada produk di kategori ini.</p>
...
export function MbahgptCategories({ site, categories, user }: CategoriesTemplateProps) {
  const parents = categories.filter((c) => !c.parent_id);
  return (
    <div data-template="mbahgpt" className="flex min-h-screen flex-col bg-background text-foreground">
      <MbahgptHeader user={user} brand={siteBrand(site)} categories={categories} />

      <main className="mx-auto w-full max-w-3xl flex-1 px-4 pt-8 pb-10 sm:pt-12">
        <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Kategori</h1>
        {parents.length === 0 ? (
          <p className="mt-4 text-sm text-[var(--muted)]">Belum ada kategori.</p>
        ) : (
