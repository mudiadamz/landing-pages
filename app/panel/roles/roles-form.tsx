...
const ROLES: { key: ConfigurableRole; label: string; noteKey: MessageKey }[] = [
  { key: "publisher", label: "Publisher", noteKey: "panel.rolePublisherNote" },
  { key: "customer", label: "Customer", noteKey: "panel.roleCustomerNote" },
];
      ...
      {ROLES.map((role) => (
        <section key={role.key} className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-6 shadow-sm">
          <div className="mb-4">
            ...
            <p className="mt-0.5 text-xs text-[var(--muted)]">{t(role.noteKey)}</p>
          </div>
